...
- **Plugins-events bridge.** The Host's `/plugins/events` Server-Sent Events channel (plugin enable/disable and rebuild notifications, including the initial full plugin graph) cannot pass through the custom protocol, which answers with complete bodies only. The shell holds that stream with the Host cookie and queues its frames (`hmr_bridge.rs`); the initialization script replaces the page's `EventSource` for this endpoint with a one-second polling command. Graph frames are full snapshots, so polling preserves the reconcile semantics — browser plugins such as the Agent Team header button now appear, disappear, and reappear with their plugin switches, matching Electron.
...
| No Windows titlebar overlay or IME menu plumbing | `main.ts` menu/window sections |
