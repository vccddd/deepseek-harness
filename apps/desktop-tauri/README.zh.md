...
| 无自定义应用菜单与 About 面板(macOS 默认 Edit 菜单在);无 Windows 标题栏 overlay 与 IME 菜单 | `main.ts` 菜单/窗口部分 |
